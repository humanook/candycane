...
#  end
     ...
     * @param mixed $value
     */
    function store(string $name, mixed $value): void
    {
        $query = $this->find('all')->where(['name' => $name]);
        if ($query->count() > 0)
        {
            $entity = $query->first();
...
//    }
    ...
    {
        $config = Configure::read('Candycane');
        $query = $this->find('all');
        if ($query->count() > 0)
        {
            foreach ($query->all()->toList() as $entity)
            {
                $k = $entity->get('name');
                $v = $entity->get('value');
                switch ($k)
        ...
        Configure::write('Candycane', $config);
        return $config;
    }
}
